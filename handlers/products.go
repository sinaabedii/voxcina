...
func AddProduct(w http.ResponseWriter, r *http.Request) {
	...
	variantsJSON := r.FormValue("variants")     // JSON array of models.ProductVariant (variant images not handled here yet)
	...
	if name == "" || priceStr == "" || categoryIDsJSON == "" || brandIDStr == "" {
		utils.ErrorResponse(w, http.StatusBadRequest, "Missing required fields: name, price, categoryIds, brandId")
		return
	}
	...
	if err := json.Unmarshal([]byte(categoryIDsJSON), &tempCategoryIDs); err != nil {
		utils.ErrorResponse(w, http.StatusBadRequest, "Invalid categoryIds JSON format: "+err.Error())
		return
	}
	for _, idStr := range tempCategoryIDs {
		objID, err := primitive.ObjectIDFromHex(idStr)
		if err != nil {
			utils.ErrorResponse(w, http.StatusBadRequest, "Invalid ObjectID in categoryIds: "+idStr)
			return
		}
	...
	if variantsJSON != "" {
		if err := json.Unmarshal([]byte(variantsJSON), &variants); err != nil {
			utils.ErrorResponse(w, http.StatusBadRequest, "Invalid variants JSON format: "+err.Error())
			return
		}
	...
	if attributesJSON != "" {
		if err := json.Unmarshal([]byte(attributesJSON), &attributes); err != nil {
			utils.ErrorResponse(w, http.StatusBadRequest, "Invalid attributes JSON format: "+err.Error())
			return
		}
	}
	...
	if err != nil {
	    utils.ErrorResponse(w, http.StatusBadRequest, "Invalid boolean value for isActive")
	    return
	}
	...
	if len(files) > MAX_MAIN_IMAGES {
	    utils.ErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("Too many main images. Maximum is %d.", MAX_MAIN_IMAGES))
	    return
	}
	...
	for _, handler := range files {
		...
		if err != nil {
			utils.ErrorResponse(w, http.StatusInternalServerError, "Error opening main image file: "+err.Error())
			return
		}
		defer file.Close()
		...
		if err := os.MkdirAll(uploadDir, os.ModePerm); err != nil {
			utils.ErrorResponse(w, http.StatusInternalServerError, "Error creating main product uploads directory: "+err.Error())
			return
		}
		...
		filename := fmt.Sprintf("%d-%s%s", time.Now().UnixNano(), strings.ReplaceAll(handler.Filename, ext, ""), ext)
		filePath := filepath.Join(uploadDir, filename)

		dst, err := os.Create(filePath)
		if err != nil {
			utils.ErrorResponse(w, http.StatusInternalServerError, "Error creating main image file on server: "+err.Error())
			return
		}
		defer dst.Close()

		if _, err := io.Copy(dst, file); err != nil {
			utils.ErrorResponse(w, http.StatusInternalServerError, "Error saving main image file: "+err.Error())
			return
		}
	...
	if err != nil {
		...
		utils.ErrorResponse(w, http.StatusInternalServerError, "Error adding product to database: "+err.Error())
		return
	}
...
func UpdateProduct(w http.ResponseWriter, r *http.Request) {
	...
	idStr, ok := vars["id"]
	if !ok {
		utils.ErrorResponse(w, http.StatusBadRequest, "Product ID not provided in URL path")
		return
	}
	...
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second) // Increased timeout for potential multiple operations
	defer cancel()
	...
	updateIfProvided := func(key string, value string, parseFunc func(string) (interface{}, error)) {
		if value != "" {
			...
			if err != nil {
				...
				utils.ErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("Invalid format for %s: %s", key, err.Error()))
	...
	existingImagePathsJSON := r.FormValue("existingImagePaths") // JSON array of strings (paths)
	if existingImagePathsJSON != "" {
		...
		if err := json.Unmarshal([]byte(existingImagePathsJSON), &existingPathsToKeep); err != nil {
			utils.ErrorResponse(w, http.StatusBadRequest, "Invalid existingImagePaths JSON format: "+err.Error())
			return
		}
		// Basic validation: ensure these paths are somewhat sane (e.g., start with /uploads/)
		for _, p := range existingPathsToKeep {
			if strings.HasPrefix(p, "/uploads/products/main/") { // Check if they are from our main upload dir
    ...
    if len(finalImagePaths)+len(files) > MAX_MAIN_IMAGES {
        utils.ErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("Total main images (existing + new) cannot exceed %d.", MAX_MAIN_IMAGES))
        return
    }
	...
	for _, handler := range files {
		file, err := handler.Open()
		if err != nil {
			utils.ErrorResponse(w, http.StatusInternalServerError, "Error opening new main image file: "+err.Error())
			return
		}
		defer file.Close()
		...
		filename := fmt.Sprintf("%s-%d-%s%s", productID.Hex(), time.Now().UnixNano(), strings.ReplaceAll(filepath.Base(handler.Filename), ext, ""), ext)
		filePath := filepath.Join(uploadDir, filename)

		dst, err := os.Create(filePath)
		if err != nil {
			utils.ErrorResponse(w, http.StatusInternalServerError, "Error creating new main image file on server: "+err.Error())
			return
		}
		defer dst.Close()
		...
		if _, err := io.Copy(dst, file); err != nil {
			...
			utils.ErrorResponse(w, http.StatusInternalServerError, "Error saving new main image file: "+err.Error())
			return
		}
		serverPath := "/uploads/products/main/" + filename
		finalImagePaths = append(finalImagePaths, serverPath)
		newlyUploadedPaths = append(newlyUploadedPaths, filePath) // Store full path for potential cleanup on DB error
		somethingToUpdate = true 
	}
    ...
    if existingImagePathsJSON != "" || len(files) > 0 {
        ...
        for _, oldPath := range existingProduct.Images {
            if !existingImageMap[oldPath] {
                imagesToDelete = append(imagesToDelete, "."+oldPath) // Add "." for server-side path
            }
        }
    }
	...
	if err != nil {
		...
		utils.ErrorResponse(w, http.StatusInternalServerError, "Error updating product: "+err.Error())
		return
	}
	...
	if err != nil {
		...
		utils.ErrorResponse(w, http.StatusInternalServerError, "Error fetching updated product: "+err.Error())
		return
	}
...
func DeleteProduct(w http.ResponseWriter, r *http.Request) {
	...
	err = collection.FindOne(ctx, bson.M{"_id": productID, "is_active": true}).Decode(&productToDeactivate)
	if err != nil {
		if err == mongo.ErrNoDocuments {
			utils.ErrorResponse(w, http.StatusNotFound, "Active product not found or already inactive")
		} else {
			utils.ErrorResponse(w, http.StatusInternalServerError, "Error fetching product: "+err.Error())
		}
		return
	...
	if err != nil {
		utils.ErrorResponse(w, http.StatusInternalServerError, "Error deactivating product: "+err.Error())
		return
	}
	...
	if result.ModifiedCount == 0 {
		...
		utils.ErrorResponse(w, http.StatusNotFound, "Product not found or no changes made (possibly already inactive)")
		return
	}
	...
	for _, imagePath := range productToDeactivate.Images {
		...
		if err := os.Remove(serverFilePath); err != nil {
			...
			fmt.Printf("WARN: Failed to delete product image %s: %v\n", serverFilePath, err)
		}
	}
	...
	utils.JSONResponse(w, http.StatusOK, map[string]string{"message": "Product deactivated and associated images marked for deletion"})
}
