...
func TestAdminEndpoints(t *testing.T) {
	...
	if api.AdminToken == "" {
		...
		if err == nil {
			...
			// Note: In a real application, there would be a secure way to create an admin
			// For testing, we're simulating having admin access
			adminEmail := "admin@example.com"
			adminPassword := "admin123"
			adminName := "Admin User"
			
			userID, token, err := api.Register(adminEmail, adminPassword, adminName)
			...
			// NOTE: We would need to update the user role to "admin" in the database
			// This would typically be done through a separate secure process
			// For testing purposes, we're assuming the first registered user in a test environment
	...
	if regularUserID != "" {
		...
		resp, _, err = api.Request(http.MethodPut, "/admin/users/"+regularUserID+"/role", updateRoleBody, api.AdminToken)
		assert.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
	...
	resp, body, err = api.Request(http.MethodPost, "/admin/categories", categoryBody, api.AdminToken)
	assert.NoError(t, err)
	...
	categoryData, ok := categoryResult["data"].(map[string]interface{})
	assert.True(t, ok)
	...
	resp, body, err = api.Request(http.MethodPost, "/admin/products", productBody, api.AdminToken)
	assert.NoError(t, err)
	assert.Equal(t, http.StatusCreated, resp.StatusCode)
	
	var productResult map[string]interface{}
	err = api.UnmarshalJSON(body, &productResult)
	assert.NoError(t, err)
	
	productData, ok := productResult["data"].(map[string]interface{})
	assert.True(t, ok)
	productID, ok := productData["id"].(string)
	assert.True(t, ok)
	
	// Test update product (admin only)
	updateProductBody := map[string]interface{}{
		"name":        "Updated Test Product",
		"description": "Updated test product description",
		"price":       129.99,
		"inventory":   200,
	}
	resp, _, err = api.Request(http.MethodPut, "/admin/products/"+productID, updateProductBody, api.AdminToken)
	assert.NoError(t, err)
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	...
	resp, body, err = api.Request(http.MethodPost, "/admin/discounts", discountBody, api.AdminToken)
	assert.NoError(t, err)
	assert.Equal(t, http.StatusCreated, resp.StatusCode)
	
	var discountResult map[string]interface{}
	err = api.UnmarshalJSON(body, &discountResult)
	assert.NoError(t, err)
	
	discountData, ok := discountResult["data"].(map[string]interface{})
	assert.True(t, ok)
	discountID, ok := discountData["id"].(string)
	assert.True(t, ok)
	
	// Test update discount (admin only)
	updateDiscountBody := map[string]interface{}{
		"discountPercent": 15,
		"isActive":        false,
	}
	resp, _, err = api.Request(http.MethodPut, "/admin/discounts/"+discountID, updateDiscountBody, api.AdminToken)
	assert.NoError(t, err)
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	
	// Test delete discount (admin only)
	resp, _, err = api.Request(http.MethodDelete, "/admin/discounts/"+discountID, nil, api.AdminToken)
	assert.NoError(t, err)
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	...
	resp, _, err = api.Request(http.MethodDelete, "/admin/products/"+productID, nil, api.AdminToken)
	assert.NoError(t, err)
	assert.Equal(t, http.StatusOK, resp.StatusCode)
} 