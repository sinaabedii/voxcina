...
func TestGetCategoryByID(t *testing.T) {
	...
	resp, body, err = api.Request(http.MethodGet, "/categories/"+categoryID, nil, "")
	assert.NoError(t, err)
	assert.Equal(t, http.StatusOK, resp.StatusCode)
...
func TestGetCategoryProducts(t *testing.T) {
	...
	resp, body, err = api.Request(http.MethodGet, "/categories/"+categoryID+"/products", nil, "")
	assert.NoError(t, err)
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	...
	// Test pagination
	resp, body, err = api.Request(http.MethodGet, "/categories/"+categoryID+"/products?page=1&limit=5", nil, "")
	assert.NoError(t, err)
	assert.Equal(t, http.StatusOK, resp.StatusCode)
} 