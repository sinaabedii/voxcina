...
//
// `Client` records the platform the pair was issued for ("android" | "web").
// It is stamped once at issuance and carried forward on every rotation from
// the STORED record — never from the header of a later request — so an
// android (permanent, non-rotating) session can never be silently downgraded
// to the web policy. An absent/empty client predates the field and follows
// the web policy.
type RefreshTokenRecord struct {
	...
	Client    string             `bson:"client,omitempty"   json:"-"` // "android" | "web" — lifetime & rotation policy
	IssuedAt  time.Time          `bson:"issued_at"          json:"-"`
	ExpiresAt time.Time          `bson:"expires_at"         json:"-"` // far-future for android; TTL index sweeps revoked rows via a re-stamp
