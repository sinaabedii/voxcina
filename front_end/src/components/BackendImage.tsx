...
export default function BackendImage({
  ...
  // If it's a backend image (starts with /uploads/), use the relative path
  // Next.js will proxy it through the rewrites in next.config.js
  const imageSrc = src.startsWith('/uploads/') 
    ? src  // Use relative path, Next.js will proxy to backend
    : src;
