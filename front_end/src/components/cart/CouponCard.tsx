      ...
      <div className="flex items-center gap-1.5">
        ...
        {!isSpent && coupon.valid_until && (
          <div className="mr-auto flex items-center bg-voxcina-blue/[0.04] dark:bg-voxcina-cream/[0.04] rounded-md px-1.5 py-0.5">
            <CountdownTimer validUntil={coupon.valid_until} onExpire={onExpire} className="text-[11px]" />
          </div>
        )}
      </div>
      ...
      {!isSpent && (
        <>
          ...
          <div className="flex items-center gap-2">
            <span className="font-mono font-bold text-xs select-all tracking-wider text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-900/30 rounded-md px-1.5 py-0.5">
              {coupon.code}
            </span>
            {!!basePrice && coupon.value > 0 && (
              <span className="text-[11px] text-emerald-600/70 dark:text-emerald-400/70 whitespace-nowrap">
            ...
            <div className="mr-auto">
              {applied ? (
                ...
                <button
                  ...
                  onClick={onApply}
                  disabled={applying}
                  className="h-8 min-w-[5rem] flex-shrink-0 whitespace-nowrap rounded-md bg-emerald-600 px-3 text-xs leading-none text-white shadow-inset-button hover:bg-emerald-700 dark:bg-emerald-500 dark:hover:bg-emerald-600 disabled:opacity-60"
                >
                  {applying ? "..." : "اعمال کد"}
                </button>
              )}
