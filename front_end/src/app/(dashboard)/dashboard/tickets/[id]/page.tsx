...
import { ArrowRight, MessageCircle, User, HeadphonesIcon, Clock } from "lucide-react";

export default function TicketDetailPage() {
  const params = useParams<{ id: string }>();
  const ticketId = params?.id as string;

  const { currentTicket, isLoading, error, fetchTicketById, addTicketMessage } = useTicketStore();
  ...
  if (isLoading && !localTicket) {
    return <div className="p-6 text-sm text-voxcina-blue/70 dark:text-voxcina-cream/70">در حال بارگذاری تیکت...</div>;
  }
  ...
  if (!localTicket) {
    return <div className="p-6 text-sm text-voxcina-blue/70 dark:text-voxcina-cream/70">تیکت پیدا نشد.</div>;
  }
    ...
    <div className="container py-8 md:py-12 mx-auto px-4 md:px-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border border-voxcina-cream dark:border-voxcina-blue/30 rounded-2xl bg-white/90 dark:bg-voxcina-blue/10 shadow-soft">
          <CardHeader>
            <CardTitle className="text-sm sm:text-base text-voxcina-blue dark:text-voxcina-cream">
              جزئیات تیکت
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-xs sm:text-sm text-voxcina-blue/80 dark:text-voxcina-cream/80">
            ...
            <div className="flex justify-between">
              <span>دسته‌بندی:</span>
              <span className="font-medium">{localTicket.category || "عمومی"}</span>
            </div>
            <div className="flex justify-between">
